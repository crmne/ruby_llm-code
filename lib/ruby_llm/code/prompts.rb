...
module RubyLLM
  module Code
    ...
    class Prompts
      ...
      def self.system_prompt(config) # rubocop:disable Metrics/PerceivedComplexity
        # Use minimal prompt for Gemini models
        if config.model.to_s.downcase.include?('gemini')
          prompt = 'You are a helpful assistant for software developers. ' \
                   'You help with coding tasks, answer technical questions, and provide creative solutions when asked.'
